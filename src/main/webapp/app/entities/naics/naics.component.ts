...
export class NaicsComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.naicsService
      ...
      .subscribe(
        (res: HttpResponse<INaics[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInNaics(): void {
    this.eventSubscriber = this.eventManager.subscribe('naicsListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: INaics[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/naics'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
