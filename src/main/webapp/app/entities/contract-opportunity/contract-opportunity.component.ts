...
export class ContractOpportunityComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.contractOpportunityService
      ...
      .subscribe(
        (res: HttpResponse<IContractOpportunity[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInContractOpportunities(): void {
    this.eventSubscriber = this.eventManager.subscribe('contractOpportunityListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IContractOpportunity[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/contract-opportunity'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.contractOpportunities = data || [];
